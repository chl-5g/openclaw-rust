...
impl Tool for FileOpsTool {
    ...
    async fn execute(
    ...
    ) -> openagentic_core::Result<serde_json::Value> {
        if !self.allowed_paths.is_empty() {
        ...
        let operation = args["operation"]
            .as_str()
            .ok_or_else(|| openagentic_core::OpenAgenticError::Tool("Missing operation".into()))?;
        ...
        match operation {
            "read" => {
                ...
                if !self.is_path_allowed(path) {
                    ...
                    return Err(openagentic_core::OpenAgenticError::Tool(
            ...
            _ => Err(openagentic_core::OpenAgenticError::Tool(
...
impl WebSearchTool {
    ...
    pub async fn search(&self, query: &str) -> openagentic_core::Result<Vec<SearchResult>> {
        match self.provider {
            SearchProvider::DuckDuckGo => self.search_duckduckgo(query).await,
            SearchProvider::SerpAPI => {
                if self.api_key.is_none() {
                    return Err(openagentic_core::OpenAgenticError::Config("SerpAPI key required".into()));
                }
                self.search_serpapi(query).await
            }
            SearchProvider::Tavily => {
                if self.api_key.is_none() {
                    return Err(openagentic_core::OpenAgenticError::Config("Tavily API key required".into()));
                }
                self.search_tavily(query).await
            }
            SearchProvider::Google => {
                if self.api_key.is_none() {
                    return Err(openagentic_core::OpenAgenticError::Config("Google API key required".into()));
                }
                self.search_google(query).await
            }
            SearchProvider::Bing => {
                if self.api_key.is_none() {
                    return Err(openagentic_core::OpenAgenticError::Config("Bing API key required".into()));
                }
    ...
    async fn search_google(&self, query: &str) -> openagentic_core::Result<Vec<SearchResult>> {
        ...
        let response = client
            ...
            .send()
            .await
            .map_err(|e| openagentic_core::OpenAgenticError::AIProvider(e.to_string()))?;
    ...
    async fn search_bing(&self, query: &str) -> openagentic_core::Result<Vec<SearchResult>> {
        let api_key = self.api_key.as_ref().unwrap();
        let url = format!(
    ...
    async fn search_duckduckgo(&self, query: &str) -> openagentic_core::Result<Vec<SearchResult>> {
        let url = format!(
    ...
    async fn search_serpapi(&self, query: &str) -> openagentic_core::Result<Vec<SearchResult>> {
        let api_key = self.api_key.as_ref().unwrap();
        let url = format!(
    ...
    async fn search_tavily(&self, query: &str) -> openagentic_core::Result<Vec<SearchResult>> {
        let api_key = self.api_key.as_ref().unwrap();
...
impl Tool for WebSearchTool {
    ...
    async fn execute(
        ...
        let query = args["query"]
            .as_str()
            .ok_or_else(|| openagentic_core::OpenAgenticError::Tool("Missing query".into()))?;
...
impl Tool for ImageGenTool {
    ...
    async fn execute(
        ...
        let prompt = args["prompt"]
            .as_str()
            .ok_or_else(|| openagentic_core::OpenAgenticError::Tool("Missing prompt".into()))?;
