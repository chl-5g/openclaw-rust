...
impl SqliteStore {
    pub fn new(path: PathBuf, table_name: &str) -> Result<Self> {
        let conn = Connection::open(&path).map_err(|e| OpenAgenticError::Config(e.to_string()))?;
    ...
    pub fn vector_search(&self, query: &SearchQuery) -> Result<Vec<SearchResult>> {
        ...
        for row in rows {
            let (id, vector_blob, payload_str) =
                row.map_err(|e| OpenAgenticError::Config(e.to_string()))?;
            let stored_vector = deserialize_vector(&vector_blob)
                .map_err(|e| OpenAgenticError::VectorStore(e.to_string()))?;
    ...
    pub fn fts_search(&self, query: &str, limit: usize) -> Result<Vec<SearchResult>> {
        ...
        for row in rows {
            let (id, payload_str) = row.map_err(|e| OpenAgenticError::Config(e.to_string()))?;
            let payload: serde_json::Value =
                serde_json::from_str(&payload_str).unwrap_or(serde_json::Value::Null);
...
impl VectorStore for SqliteStore {
    ...
    async fn delete_by_filter(&self, _filter: Filter) -> Result<usize> {
        Err(OpenAgenticError::Config("not implemented".to_string()))
    }
...
fn deserialize_vector(blob: &[u8]) -> Result<Vec<f32>> {
    if blob.len() % 4 != 0 {
        return Err(OpenAgenticError::VectorStore(format!(
