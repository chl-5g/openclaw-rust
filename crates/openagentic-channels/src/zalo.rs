...
impl ZaloClient {
    ...
    pub async fn get_user_info(&self, user_id: &str) -> Result<ZaloUserInfo> {
        ...
        let response = self
            ...
            .send()
            .await
            .map_err(|e| OpenAgenticError::Http(format!("Zalo API 错误: {}", e)))?;
        ...
        response
            .json()
            .await
            .map_err(|e| OpenAgenticError::Http(format!("解析 Zalo 响应失败: {}", e)))
    }
...
impl Channel for ZaloClient {
    ...
    async fn start(&mut self) -> Result<()> {
        if self.config.access_token.is_none() {
            return Err(OpenAgenticError::Channel("Zalo access token 未配置".into()));
        }
