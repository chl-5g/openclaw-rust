...
impl DiscordChannel {
    ...
    pub async fn start_gateway(&mut self) -> Result<()> {
        ...
        let (client, receiver): (DiscordGatewayClient, mpsc::UnboundedReceiver<DiscordGatewayEvent>) = 
            ...
            .await
            .map_err(|e| OpenAgenticError::Config(format!("Failed to create Discord Gateway: {}", e)))?;
    ...
    pub async fn start_gateway_with_message_sender_with_receiver(
        ...
        if !self.config.use_gateway {
            return Err(OpenAgenticError::Config("Gateway not enabled".to_string()));
        }
    ...
    pub async fn send_to_channel(&self, channel_id: &str, content: &str) -> Result<()> {
        ...
        let response = self
            ...
            .send()
            .await
            .map_err(|e| OpenAgenticError::Http(format!("Discord API 请求失败: {}", e)))?;
    ...
    pub async fn send_webhook(&self, content: &str, username: Option<&str>) -> Result<()> {
        let webhook_url = self
            ...
            .webhook_url
            .as_ref()
            .ok_or_else(|| OpenAgenticError::Config("未配置 Webhook URL".to_string()))?;
        ...
        let response = self
            ...
            .send()
            .await
            .map_err(|e| OpenAgenticError::Http(format!("Discord Webhook 请求失败: {}", e)))?;
...
impl Channel for DiscordChannel {
    ...
    async fn start(&mut self) -> Result<()> {
        if !self.config.enabled {
            return Err(OpenAgenticError::Config("Discord 通道未启用".to_string()));
        }
