...
"#,
        ...
        let output = Command::new("osascript")
            ...
            .arg(&script)
            .output()
            .map_err(|e| OpenAgenticError::Execution(format!("执行 AppleScript 失败: {}", e)))?;
...
"#,
        ...
        if output.status.success() {
            ...
            Err(OpenAgenticError::Execution(format!("短信发送失败: {}", error)))
        }
    }

    #[cfg(not(target_os = "macos"))]
    fn send_sms_fallback(&self, _phone: &str, _message: &str) -> Result<String> {
        Err(OpenAgenticError::Platform("短信仅支持 macOS 系统".to_string()))
    }
...
"#;
        ...
        let output = Command::new("osascript")
            ...
            .arg(script)
            .output()
            .map_err(|e| OpenAgenticError::Execution(format!("获取消息失败: {}", e)))?;
...
impl Channel for IMessageChannel {
    ...
    async fn start(&mut self) -> Result<()> {
        if !Self::check_macos() {
            ...
            return Err(OpenAgenticError::Platform("iMessage 仅支持 macOS".to_string()));
        }
...
"#,
        ...
        let output = Command::new("osascript")
            ...
            .arg(&script)
            .output()
            .map_err(|e| OpenAgenticError::Execution(format!("发送附件失败: {}", e)))?;
        ...
        if output.status.success() {
            ...
            Err(OpenAgenticError::Execution(format!("发送附件失败: {}", error)))
        }
    }
    ...
    pub async fn send_with_attachment(
        ...
        Err(OpenAgenticError::Platform("iMessage 仅支持 macOS".to_string()))
    }
