...
impl MatrixClient {
    ...
    async fn request<T: for<'de> Deserialize<'de>>(
        ...
        let response = request
            .send()
            .await
            .map_err(|e| OpenAgenticError::Http(format!("Matrix API 错误: {}", e)))?;
        ...
        response
            .json()
            .await
            .map_err(|e| OpenAgenticError::Http(format!("解析 Matrix 响应失败: {}", e)))
    }
    ...
    pub async fn login(&mut self, username: &str, password: &str) -> Result<MatrixLoginResponse> {
        let body = serde_json::json!({
            ...
            "password": password,
            "initial_device_display_name": "OpenAgentic"
        });
    ...
    pub async fn sync(&self, timeout: u64) -> Result<MatrixSyncResponse> {
        ...
        let query = serde_qs::to_string(&params)
            .map_err(|e| OpenAgenticError::Config(format!("构建查询失败: {}", e)))?;
...
impl Channel for MatrixClient {
    ...
    async fn start(&mut self) -> Result<()> {
        if self.config.access_token.is_none() {
            return Err(OpenAgenticError::Channel(
