...
impl SlackChannel {
    ...
    pub async fn send_webhook(&self, text: &str, blocks: Option<Vec<SlackBlock>>) -> Result<()> {
        ...
        let response = self
            ...
            .send()
            .await
            .map_err(|e| OpenAgenticError::Http(format!("Slack Webhook 请求失败: {}", e)))?;
    ...
    pub async fn send_to_channel(
        ...
        let bot_token = self
            ...
            .bot_token
            .as_ref()
            .ok_or_else(|| OpenAgenticError::Config("未配置 Bot Token".to_string()))?;
        ...
        let response = self
            ...
            .send()
            .await
            .map_err(|e| OpenAgenticError::Http(format!("Slack API 请求失败: {}", e)))?;
...
impl Channel for SlackChannel {
    ...
    async fn start(&mut self) -> Result<()> {
        if !self.config.enabled {
            return Err(OpenAgenticError::Config("Slack 通道未启用".to_string()));
        }
        Ok(())
    }
