...
impl GoogleChatClient {
    ...
    pub async fn send_message(&self, space: &str, text: &str) -> Result<GoogleChatSendResponse> {
        ...
        let response = self
            ...
            .send()
            .await
            .map_err(|e| OpenAgenticError::Http(format!("Google Chat API 错误: {}", e)))?;
        ...
        response
            .json()
            .await
            .map_err(|e| OpenAgenticError::Http(format!("解析 Google Chat 响应失败: {}", e)))
    }
    ...
    pub async fn handle_webhook(&self, payload: serde_json::Value) -> Result<ChannelMessage> {
        let msg: GoogleChatMessage =
            serde_json::from_value(payload).map_err(OpenAgenticError::Serialization)?;
