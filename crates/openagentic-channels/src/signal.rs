...
impl SignalChannel {
    ...
    async fn send_text(&self, to: &str, message: &str) -> Result<SignalMessageResponse> {
        ...
        let response = self
            ...
            .send()
            .await
            .map_err(|e| OpenAgenticError::Network(format!("Signal API 请求失败: {}", e)))?;

        if !response.status().is_success() {
            let error_text = response.text().await.unwrap_or_default();
            return Err(OpenAgenticError::Api(format!(
                "Signal 发送失败: {}",
                error_text
            )));
        }
        ...
        response
            ...
            .await
            .map_err(|e| OpenAgenticError::Parse(format!("解析响应失败: {}", e)))
    }
    ...
    async fn get_messages(&self) -> Result<Vec<SignalMessage>> {
        ...
        response
            ...
            .await
            .map_err(|e| OpenAgenticError::Parse(format!("解析消息失败: {}", e)))
    }
...
impl SignalChannel {
    ...
    pub async fn list_groups(&self) -> Result<Vec<SignalGroup>> {
        ...
        let response = self
            ...
            .send()
            .await
            .map_err(|e| OpenAgenticError::Network(format!("获取群组失败: {}", e)))?;

        response
            .json::<Vec<SignalGroup>>()
            .await
            .map_err(|e| OpenAgenticError::Parse(format!("解析群组失败: {}", e)))
    }
    ...
    pub async fn create_group(&self, name: &str, members: &[String]) -> Result<SignalGroup> {
        ...
        let response = self
            ...
            .send()
            .await
            .map_err(|e| OpenAgenticError::Network(format!("创建群组失败: {}", e)))?;
    ...
    pub async fn send_group_message(
        ...
        let response = self
            ...
            .send()
            .await
            .map_err(|e| OpenAgenticError::Network(format!("发送群组消息失败: {}", e)))?;
