...
impl WeComChannel {
    ...
    async fn send_request(&self, body: &serde_json::Value) -> Result<()> {
        let response = self
            ...
            .send()
            .await
            .map_err(|e| OpenAgenticError::Http(format!("企业微信 API 请求失败: {}", e)))?;
...
impl Channel for WeComChannel {
    ...
    async fn start(&mut self) -> Result<()> {
        if !self.config.enabled {
            return Err(OpenAgenticError::Config("企业微信通道未启用".to_string()));
        }
        Ok(())
    }
        ...
        match message.message_type.as_str() {
            ...
            "image" => {
                let base64 = message
                    .base64
                    .as_deref()
                    .ok_or_else(|| OpenAgenticError::Config("图片消息需要 base64 字段".to_string()))?;
                let md5 = message
                    .md5
                    .as_deref()
                    .ok_or_else(|| OpenAgenticError::Config("图片消息需要 md5 字段".to_string()))?;
                self.send_image(base64, md5).await?;
            }
            "news" => {
                let articles = message.articles.ok_or_else(|| {
                    OpenAgenticError::Config("图文消息需要 articles 字段".to_string())
                })?;
                self.send_news(articles).await?;
            }
            "file" => {
                let media_id = message.media_id.as_deref().ok_or_else(|| {
                    OpenAgenticError::Config("文件消息需要 media_id 字段".to_string())
                })?;
