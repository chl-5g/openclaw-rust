...
impl SmsClient {
    ...
    async fn send_twilio(&self, to: &str, body: &str) -> Result<SmsResponse> {
        let account_sid = self
            ...
            .api_key
            .as_ref()
            .ok_or_else(|| OpenAgenticError::Channel("Twilio Account SID 未配置".into()))?;
        let auth_token = self
            .config
            .api_secret
            .as_ref()
            .ok_or_else(|| OpenAgenticError::Channel("Twilio Auth Token 未配置".into()))?;
        ...
        let response = self
            ...
            .send()
            .await
            .map_err(|e| OpenAgenticError::Http(format!("Twilio API 错误: {}", e)))?;

        if !response.status().is_success() {
            let status = response.status();
            let error_text = response.text().await.unwrap_or_default();
            return Err(OpenAgenticError::Http(format!(
                "Twilio API 错误 ({}): {}",
                status, error_text
            )));
        }
        ...
        response
            .json()
            .await
            .map_err(|e| OpenAgenticError::Http(format!("解析 Twilio 响应失败: {}", e)))
    }
    ...
    async fn send_nexmo(&self, to: &str, body: &str) -> Result<SmsResponse> {
        let api_key = self
            ...
            .api_key
            .as_ref()
            .ok_or_else(|| OpenAgenticError::Channel("Nexmo API Key 未配置".into()))?;
        let api_secret = self
            .config
            .api_secret
            .as_ref()
            .ok_or_else(|| OpenAgenticError::Channel("Nexmo API Secret 未配置".into()))?;
        ...
        let body = serde_json::json!({
            ...
            "from": self.config.from_number.as_deref().unwrap_or("OpenAgentic"),
            "to": to,
            "text": body
        });
        ...
        let response = self
            ...
            .send()
            .await
            .map_err(|e| OpenAgenticError::Http(format!("Nexmo API 错误: {}", e)))?;
        ...
        response
            .json()
            .await
            .map_err(|e| OpenAgenticError::Http(format!("解析 Nexmo 响应失败: {}", e)))
    }
    ...
    async fn send_custom(&self, to: &str, body: &str) -> Result<SmsResponse> {
        let base_url = self
            ...
            .base_url
            .as_ref()
            .ok_or_else(|| OpenAgenticError::Channel("自定义 SMS 提供商 URL 未配置".into()))?;
        ...
        let response = self
            ...
            .send()
            .await
            .map_err(|e| OpenAgenticError::Http(format!("SMS API 错误: {}", e)))?;
        ...
        response
            .json()
            .await
            .map_err(|e| OpenAgenticError::Http(format!("解析 SMS 响应失败: {}", e)))
    }
}
...
impl Channel for SmsClient {
    ...
    async fn start(&mut self) -> Result<()> {
        if self.config.api_key.is_none() {
            return Err(OpenAgenticError::Channel("SMS API Key 未配置".into()));
        }
    ...
    async fn send(&self, message: SendMessage) -> Result<ChannelMessage> {
        ...
        if let Some(error_code) = response.error_code {
            return Err(OpenAgenticError::Channel(format!(
                "SMS 发送失败: {}",
