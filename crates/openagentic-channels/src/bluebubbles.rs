...
impl BlueBubblesClient {
    ...
    pub async fn send_message(
        ...
        let response = request
            .send()
            .await
            .map_err(|e| OpenAgenticError::Http(format!("BlueBubbles API 错误: {}", e)))?;
        ...
        response
            .json()
            .await
            .map_err(|e| OpenAgenticError::Http(format!("解析 BlueBubbles 响应失败: {}", e)))
    }
...
impl Channel for BlueBubblesClient {
    ...
    async fn send(&self, message: SendMessage) -> Result<ChannelMessage> {
        ...
        let msg_result = response
            .result
            .ok_or_else(|| OpenAgenticError::Channel("解析消息响应失败".into()))?;
