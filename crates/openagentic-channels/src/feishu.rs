...
impl FeishuChannel {
    ...
    pub async fn get_access_token(&mut self) -> Result<String> {
        ...
        let response = self
            ...
            .send()
            .await
            .map_err(|e| OpenAgenticError::Http(format!("飞书 API 请求失败: {}", e)))?;
    ...
    /// 飞书 @消息格式:
    /// - `@{机器人名称}` 或 `@机器人名称`
    /// - 示例: `@OpenAgentic 你好`, `@goclaw 帮我写个函数`
    ...
    pub async fn send_text_webhook(&self, content: &str) -> Result<()> {
        let webhook = self
            ...
            .webhook
            .as_ref()
            .ok_or_else(|| OpenAgenticError::Config("未配置 Webhook 地址".to_string()))?;
        ...
        match event_type {
            "im.message" => {
                let message = event_json.get("message").ok_or_else(|| {
                    OpenAgenticError::AIProvider("无效的消息事件".to_string())
                })?;
                Ok(self.parse_event_message(&serde_json::json!({ "message": message })))
            }
            _ => Err(OpenAgenticError::AIProvider(format!(
                "未支持的事件类型: {}",
                event_type
            ))),
        }
...
impl Channel for FeishuChannel {
    ...
    async fn start(&mut self) -> Result<()> {
        if !self.config.enabled {
            return Err(OpenAgenticError::Config("飞书通道未启用".to_string()));
        }
        Ok(())
    }
