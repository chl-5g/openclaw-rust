...
impl DingTalkChannel {
    ...
    async fn send_request(&self, body: &serde_json::Value) -> Result<()> {
        ...
        let response = self
            ...
            .send()
            .await
            .map_err(|e| OpenAgenticError::Http(format!("钉钉 API 请求失败: {}", e)))?;
    ...
    fn sign(&self, secret: &str, timestamp: i64) -> Result<String> {
        ...
        let mut mac = Hmac::<Sha256>::new_from_slice(secret.as_bytes())
            .map_err(|e| OpenAgenticError::Config(format!("HMAC 初始化失败: {}", e)))?;
...
impl Channel for DingTalkChannel {
    ...
    async fn start(&mut self) -> Result<()> {
        if !self.config.enabled {
            return Err(OpenAgenticError::Config("钉钉通道未启用".to_string()));
        }
        Ok(())
    }
        ...
        match message.message_type.as_str() {
            ...
            "link" => {
                ...
                let url = message
                    .url
                    .as_deref()
                    .ok_or_else(|| OpenAgenticError::Config("链接消息需要 url 字段".to_string()))?;
