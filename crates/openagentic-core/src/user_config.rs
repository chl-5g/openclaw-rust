...
impl UserConfig {
    ...
    pub fn from_file(path: &PathBuf) -> crate::Result<Self> {
        let content = std::fs::read_to_string(path)
            .map_err(|e| crate::OpenAgenticError::Config(format!("读取用户配置失败: {}", e)))?;

        let config: UserConfig = serde_json::from_str(&content)
            .map_err(|e| crate::OpenAgenticError::Config(format!("解析用户配置失败: {}", e)))?;
    ...
    pub fn save(&self, path: &PathBuf) -> crate::Result<()> {
        ...
        if let Some(parent) = path.parent() {
            std::fs::create_dir_all(parent)
                .map_err(|e| crate::OpenAgenticError::Config(format!("创建配置目录失败: {}", e)))?;
        }
    ...
    pub fn default_config_path() -> PathBuf {
        ...
        home.join(".open-agentic").join("user_config.json")
    }
