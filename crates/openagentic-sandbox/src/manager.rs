...
use openagentic_security::{GrantResult, NetworkDecision, SecurityMiddleware};
...
impl SandboxManager {
    ...
    async fn check_security(
        ...
        if !filter_result.allowed
            && filter_result.threat_level >= openagentic_security::ThreatLevel::High
        {
    ...
    async fn execute_docker(
        ...
        if let Some(ref client) = *docker {
            ...
            let sandbox_config = SandboxConfig {
                image: "openagentic/tool-executor:latest".to_string(),
