...
impl CustomProvider {
    ...
    /// # Example
    /// ```rust
    /// use openagentic_ai::providers::CustomProvider;
    ///
...
impl CustomProviderBuilder {
    ...
    pub fn build(self) -> Result<CustomProvider> {
        let base_url = self
            .base_url
            .ok_or_else(|| openagentic_core::OpenAgenticError::Config("base_url is required".into()))?;

        let api_key = self
            .api_key
            .ok_or_else(|| openagentic_core::OpenAgenticError::Config("api_key is required".into()))?;
