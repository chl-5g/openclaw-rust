...
use openagentic_core::{Message, OpenAgenticError, Result, Role};
...
impl AIProvider for AnthropicProvider {
    ...
    async fn chat(&self, request: ChatRequest) -> Result<ChatResponse> {
        ...
        let response = self
            ...
            .send()
            .await
            .map_err(|e| OpenAgenticError::Http(format!("Anthropic API 请求失败: {}", e)))?;
        ...
        let json: serde_json::Value = response
            .json()
            .await
            .map_err(|e| OpenAgenticError::AIProvider(format!("解析响应失败: {}", e)))?;
    ...
    async fn chat_stream(
        ...
        let response = self
            ...
            .send()
            .await
            .map_err(|e| OpenAgenticError::Http(format!("Anthropic Stream API 请求失败: {}", e)))?;
    ...
    async fn embed(&self, _request: EmbeddingRequest) -> Result<EmbeddingResponse> {
        ...
        Err(OpenAgenticError::AIProvider(
...
impl AnthropicProvider {
        ...
        async_stream::stream! {
            ...
            while let Some(bytes_result) = byte_stream.next().await {
                match bytes_result {
                    ...
                    Err(e) => {
                        yield Err(OpenAgenticError::Http(format!("Anthropic 流读取错误: {}", e)));
                        return;
                    }
                }
            }
    ...
    fn parse_anthropic_sse_event(
        ...
        let json: serde_json::Value = match serde_json::from_str(&data) {
            Ok(j) => j,
            Err(e) => return Some(Err(OpenAgenticError::Serialization(e))),
        };
        ...
        match event_type.as_str() {
            ...
            "error" => {
                ...
                Some(Err(OpenAgenticError::AIProvider(format!(
                    "Anthropic API 错误: {}",
                    error_msg
                ))))
            }
