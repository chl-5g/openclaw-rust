...
impl ToolCall {
    ...
    pub fn parse_arguments<T: for<'de> Deserialize<'de>>(&self) -> Result<T> {
        serde_json::from_str(&self.function.arguments)
            .map_err(openagentic_core::OpenAgenticError::Serialization)
    }
}
...
impl ToolRegistry {
    ...
    pub async fn execute(&self, name: &str, arguments: Value) -> Result<String> {
        let tool = self.get(name).ok_or_else(|| {
            openagentic_core::OpenAgenticError::Unknown(format!("Tool not found: {}", name))
        })?;
...
impl ToolExecutor for CalculatorTool {
    ...
    async fn execute(&self, arguments: Value) -> Result<String> {
        let expr = arguments["expression"]
            .as_str()
            .ok_or_else(|| openagentic_core::OpenAgenticError::Unknown("Missing expression".into()))?;
...
impl ToolExecutor for WeatherTool {
    ...
    async fn execute(&self, arguments: Value) -> Result<String> {
        let city = arguments["city"]
            .as_str()
            .ok_or_else(|| openagentic_core::OpenAgenticError::Unknown("Missing city".into()))?;
