...
pub trait TokenCounter: Send + Sync {
    ...
    fn count_message(&self, message: &Message) -> usize {
        ...
        let role_tokens = match message.role {
            openagentic_core::Role::System => 4,
            openagentic_core::Role::User => 4,
            openagentic_core::Role::Assistant => 4,
            openagentic_core::Role::Tool => 4,
        };
        ...
        let content_tokens: usize = message
            ...
            .iter()
            .map(|c| match c {
                openagentic_core::Content::Text { text } => self.count(text),
                openagentic_core::Content::Image { .. } => 85,
                openagentic_core::Content::Audio { .. } => 0,
                openagentic_core::Content::ToolCall { arguments, .. } => {
                    self.count(&arguments.to_string()) + 10
                }
                openagentic_core::Content::ToolResult { content, .. } => self.count(content),
            })
            .sum();
...
impl TiktokenCounter {
    pub fn new(encoder: &str) -> Result<Self> {
        ...
        .map_err(|e| openagentic_core::OpenAgenticError::TokenCount(format!("加载分词器失败: {}", e)))?;
