...
use openagentic_server::gateway_service::Gateway;
use openagentic_server::server_config::ServerConfig;
use std::path::PathBuf;

fn get_config_dir() -> Option<PathBuf> {
    dirs::home_dir().map(|p| p.join(".open-agentic"))
}
...
pub async fn run(
    ...
    tracing::info!("Starting OpenAgentic Gateway...");
    ...
    let gateway: openagentic_server::gateway_service::Gateway = Gateway::new(config).await?;
    gateway.start().await?;
