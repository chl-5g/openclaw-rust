...
use clap::Subcommand;
use openagentic_core::{OpenAgenticError, UserConfigManager, UserProviderConfig};
    ...
    pub async fn execute(&self) -> Result<(), OpenAgenticError> {
        ...
        match self {
            ApiKeyCommand::Set {
                ...
                // 验证 API Key 格式
                if !openagentic_core::UserConfig::validate_api_key(provider, api_key)? {
            ...
            ApiKeyCommand::List => {
                ...
                if providers.is_empty() {
                    println!("暂无配置的提供商");
                    println!("\n使用方法:");
                    println!("  openagentic api-key set openai sk-xxx");
                } else {
                    println!("已配置的提供商:");
                    println!();
            ...
            ApiKeyCommand::Validate { provider, api_key } => {
                match openagentic_core::UserConfig::validate_api_key(provider, api_key) {
