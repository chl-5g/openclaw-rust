...
pub struct OnboardConfig {
    ...
    pub channels: openagentic_channels::ChannelConfigs,
...
pub async fn run(quick: bool) -> Result<()> {
    println!();
    println!("\x1b[36m\x1b[1m🦀 Welcome to OpenAgentic!\x1b[0m");
    println!();
    ...
    println!("  • \x1b[36mopen-agentic doctor\x1b[0m - Check system health");
    println!("  • \x1b[36mopen-agentic gateway\x1b[0m - Start the service");
    println!();
...
async fn run_interactive_mode(config: &mut OnboardConfig) -> Result<()> {
    ...
    if selected_channels.contains(&2) {
        println!("  \x1b[2mWhatsApp: Run 'open-agentic channels login whatsapp' after setup\x1b[0m");
    }
