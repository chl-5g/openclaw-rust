...
impl Default for DaemonConfig {
    fn default() -> Self {
        Self {
            name: "openagentic".to_string(),
            executable: "open-agentic".to_string(),
...
impl DaemonConfig {
    ...
    pub fn gateway(port: u16, host: &str) -> Self {
        Self {
            name: "openagentic-gateway".to_string(),
            executable: "open-agentic".to_string(),
            args: vec![
                "gateway".to_string(),
                "--port".to_string(),
...
impl LaunchAgentConfig {
    ...
    pub fn from_daemon_config(config: &DaemonConfig) -> Self {
        ...
        Self {
            label: format!("com.openagentic.{}", config.name),
...
impl SystemdServiceConfig {
    ...
    pub fn from_daemon_config(config: &DaemonConfig) -> Self {
        ...
        Self {
            ...
            description: format!("OpenAgentic {}", config.name),
            exec_start,
            working_directory: config
                .working_dir
                .as_ref()
...
fn get_default_pid_path() -> PathBuf {
    ...
    PathBuf::from(format!("{}/.open-agentic/openagentic.pid", home))
}
...
mod tests {
    ...
    fn test_daemon_config_default() {
        ...
        assert_eq!(config.name, "openagentic");
    ...
    fn test_launch_agent_config() {
        let config = DaemonConfig::default();
        let launch_agent = LaunchAgentConfig::from_daemon_config(&config);
        assert!(launch_agent.label.starts_with("com.openagentic."));
    }
}
