...
impl crate::tts::TextToSpeech for CustomTtsProvider {
    ...
    async fn synthesize(
    ...
    ) -> openagentic_core::Result<Vec<u8>> {
        ...
        let mut request = match self.config.method.to_uppercase().as_str() {
            ...
            _ => {
                return Err(openagentic_core::OpenAgenticError::Config(format!(
                    "不支持的 HTTP 方法: {}",
                    self.config.method
                )));
            }
        ...
        let response = request
            .send()
            .await
            .map_err(|e| openagentic_core::OpenAgenticError::Http(format!("请求失败: {}", e)))?;

        if !response.status().is_success() {
            return Err(openagentic_core::OpenAgenticError::AIProvider(format!(
                "API 错误: {}",
                response.status()
            )));
        }
        ...
        match self.config.response_type {
            CustomResponseType::Binary => {
                let bytes = response.bytes().await.map_err(|e| {
                    openagentic_core::OpenAgenticError::Http(format!("读取响应失败: {}", e))
                })?;
                Ok(bytes.to_vec())
            }
            CustomResponseType::Json { ref audio_field } => {
                let json: serde_json::Value = response.json().await.map_err(|e| {
                    openagentic_core::OpenAgenticError::Http(format!("解析 JSON 失败: {}", e))
                })?;
                ...
                let audio_base64 =
                    json.get(audio_field)
                        .and_then(|v| v.as_str())
                        .ok_or_else(|| {
                            openagentic_core::OpenAgenticError::Config("响应中未找到音频字段".to_string())
                        })?;

                let audio_data = base64::decode(audio_base64).map_err(|e| {
                    openagentic_core::OpenAgenticError::Config(format!("Base64 解码失败: {}", e))
                })?;
...
impl crate::stt::SpeechToText for CustomSttProvider {
    ...
    async fn transcribe(
    ...
    ) -> openagentic_core::Result<TranscriptionResult> {
        ...
        let part = reqwest::multipart::Part::bytes(audio_data.to_vec())
            ...
            .mime_str("audio/wav")
            .map_err(|e| {
                openagentic_core::OpenAgenticError::Http(format!("创建 multipart 失败: {}", e))
            })?;
