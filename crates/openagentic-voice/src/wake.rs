...
fn zip_err(e: zip::result::ZipError) -> OpenAgenticError {
    OpenAgenticError::Io(std::io::Error::other(e.to_string()))
}
...
impl VoskWakeDetector {
    ...
    pub async fn download_model(model_type: VoskModelType) -> Result<String> {
        ...
        std::fs::create_dir_all(&models_dir)
            .map_err(|e| OpenAgenticError::Config(format!("创建模型目录失败: {}", e)))?;

        // 下载并解压
        let response = reqwest::get(url)
            .await
            .map_err(|e| OpenAgenticError::Http(format!("下载模型失败: {}", e)))?;

        let bytes = response
            .bytes()
            .await
            .map_err(|e| OpenAgenticError::Http(format!("读取模型数据失败: {}", e)))?;
...
impl VoiceWake for VoskWakeDetector {
    async fn start(&mut self) -> Result<()> {
        if self.model_path.is_none() {
            return Err(OpenAgenticError::Config("Vosk 需要指定模型路径".to_string()));
        }
...
mod tests {
    ...
    fn test_check_wake_word() {
        ...
        let result = detector.check_wake_word("hey openagentic");
        assert!(result.is_some(), "Should detect wake word 'hey openagentic'");
        if let Some((word, confidence)) = result {
            assert_eq!(word, "hey openagentic");
            assert_eq!(confidence, 1.0);
        }

        let result = detector.check_wake_word("Hey OpenAgentic, what time is it?");
    ...
    fn test_check_wake_word_threshold() {
        let config = WakeWordConfig {
            wake_words: vec!["hey openagentic".to_string()],
            threshold: 0.9,
            min_audio_length_ms: 500,
            sensitivity: 0.5,
        };
