...
impl VoiceAgentBuilder {
    ...
    pub fn build(self) -> Result<VoiceAgent> {
        let stt = self.stt.ok_or_else(|| openagentic_core::OpenAgenticError::Config("STT not configured".to_string()))?;
        let tts = self.tts.ok_or_else(|| openagentic_core::OpenAgenticError::Config("TTS not configured".to_string()))?;
