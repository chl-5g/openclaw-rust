...
pub trait TextToSpeech: Send + Sync {
    ...
    async fn synthesize_to_file(
        ...
        std::fs::write(output_path, audio_data)
            .map_err(|e| OpenAgenticError::Config(format!("写入音频文件失败: {}", e)))?;
        Ok(())
    }
...
impl OpenAITts {
    ...
    async fn get_api_key(&self) -> Result<String> {
        self.config
            .get_openai_api_key()
            .await
            .map_err(|e| OpenAgenticError::Config(e))
    }
...
impl TextToSpeech for OpenAITts {
    ...
    async fn synthesize(&self, text: &str, options: Option<SynthesisOptions>) -> Result<Vec<u8>> {
        ...
        let response = self
            ...
            .send()
            .await
            .map_err(|e| OpenAgenticError::Http(format!("TTS API 请求失败: {}", e)))?;

        if !response.status().is_success() {
            let error_text = response.text().await.unwrap_or_default();
            return Err(OpenAgenticError::AIProvider(format!(
                "TTS API 错误: {}",
                error_text
            )));
        }
        ...
        let audio_data = response
            .bytes()
            .await
            .map_err(|e| OpenAgenticError::Http(format!("读取音频数据失败: {}", e)))?;
...
impl TextToSpeech for ElevenLabsTts {
    ...
    async fn synthesize(&self, text: &str, options: Option<SynthesisOptions>) -> Result<Vec<u8>> {
        ...
        let response = self
            ...
            .send()
            .await
            .map_err(|e| OpenAgenticError::Config(format!("ElevenLabs 请求失败: {}", e)))?;

        if !response.status().is_success() {
            let status = response.status();
            let error_text = response.text().await.unwrap_or_default();
            return Err(OpenAgenticError::Config(format!(
                "ElevenLabs API 错误 ({}): {}",
                status, error_text
            )));
        }
        ...
        let audio_data = response
            .bytes()
            .await
            .map_err(|e| OpenAgenticError::Config(format!("读取音频数据失败: {}", e)))?;
...
impl AzureTts {
    ...
    fn get_endpoint(&self) -> Result<String> {
        let region = self
            ...
            .azure_region
            .as_ref()
            .ok_or_else(|| OpenAgenticError::Config("Azure region 未配置".to_string()))?;
        Ok(format!(
    ...
    fn get_api_key(&self) -> Result<String> {
        self.config
            .azure_api_key
            .clone()
            .ok_or_else(|| OpenAgenticError::Config("Azure API Key 未配置".to_string()))
    }
}
...
impl TextToSpeech for AzureTts {
    ...
    async fn synthesize(&self, text: &str, options: Option<SynthesisOptions>) -> Result<Vec<u8>> {
        ...
        let response = self
            ...
            .send()
            .await
            .map_err(|e| OpenAgenticError::Http(format!("Azure TTS 请求失败: {}", e)))?;
...
impl GoogleTts {
    ...
    fn get_api_key(&self) -> Result<String> {
        self.config
            .google_api_key
            .clone()
            .ok_or_else(|| OpenAgenticError::Config("Google API Key 未配置".to_string()))
    }
}
...
impl TextToSpeech for GoogleTts {
    ...
    async fn synthesize(&self, text: &str, options: Option<SynthesisOptions>) -> Result<Vec<u8>> {
        ...
        let response = self
            ...
            .send()
            .await
            .map_err(|e| OpenAgenticError::Http(format!("Google TTS 请求失败: {}", e)))?;
        ...
        let json: serde_json::Value = response
            .json()
            .await
            .map_err(|e| OpenAgenticError::Http(format!("解析响应失败: {}", e)))?;

        let audio_content = json
            .get("audioContent")
            .and_then(|v| v.as_str())
            .ok_or_else(|| OpenAgenticError::Config("响应中未找到 audioContent".to_string()))?;

        let audio_data = base64::engine::general_purpose::STANDARD
            .decode(audio_content)
            .map_err(|e| OpenAgenticError::Config(format!("Base64 解码失败: {}", e)))?;
