...
impl LocalWhisperStt {
    ...
    pub fn load_model(&mut self) -> Result<()> {
        ...
        let context = WhisperContext::new(&self.config.model_path)
            .map_err(|e| OpenAgenticError::Config(format!("加载 Whisper 模型失败: {}", e)))?;
    ...
    pub async fn download_model(model_type: WhisperModelType) -> Result<String> {
        ...
        std::fs::write(&model_path, &bytes)
            .map_err(|e| OpenAgenticError::Config(format!("保存模型失败: {}", e)))?;
        ...
        #[cfg(feature = "local-whisper")]
        {
            ...
            if ctx_guard.is_none() {
                let ctx = WhisperContext::new(&self.config.model_path)
                    .map_err(|e| OpenAgenticError::Config(format!("加载模型失败: {}", e)))?;
                *ctx_guard = Some(ctx);
            }
            ...
            let mut state = context
                .create_state()
                .map_err(|e| OpenAgenticError::Config(format!("创建状态失败: {}", e)))?;
            
            state
                .full(params, &samples)
                .map_err(|e| OpenAgenticError::Config(format!("转写失败: {}", e)))?;
            
            let num_segments = state
                .full_n_segments()
                .map_err(|e| OpenAgenticError::Config(format!("获取段落数失败: {}", e)))?;
