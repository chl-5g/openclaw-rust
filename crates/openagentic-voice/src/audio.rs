...
impl AudioRecorder {
    ...
    pub fn start_recording(&self) -> Result<AudioRecording> {
        ...
        let device = host
            ...
            .ok_or_else(|| OpenAgenticError::Config("找不到音频输入设备".to_string()))?;

        let config = device
            .default_input_config()
            .map_err(|e| OpenAgenticError::Config(format!("获取音频配置失败: {}", e)))?;
        ...
        let stream = match config.sample_format() {
            ...
            _ => {
                return Err(OpenAgenticError::Config("不支持的音频格式".to_string()));
            }
        }
        .map_err(|e| OpenAgenticError::Config(format!("创建录音流失败: {}", e)))?;

        stream
            .play()
            .map_err(|e| OpenAgenticError::Config(format!("开始录音失败: {}", e)))?;
...
impl AudioPlayer {
    ...
    pub fn play(&self, audio_data: &[u8]) -> Result<()> {
        ...
        let device = host
            ...
            .ok_or_else(|| OpenAgenticError::Config("找不到音频输出设备".to_string()))?;
        ...
        let stream = device
            ...
            .map_err(|e| OpenAgenticError::Config(format!("创建播放流失败: {}", e)))?;

        stream
            .play()
            .map_err(|e| OpenAgenticError::Config(format!("开始播放失败: {}", e)))?;
...
impl AudioUtils {
    ...
    pub fn list_input_devices() -> Result<Vec<String>> {
        ...
        let devices = host
            .input_devices()
            .map_err(|e| OpenAgenticError::Config(format!("获取设备列表失败: {}", e)))?;
