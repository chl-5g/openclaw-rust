    ...
    fn from(backend: &str) -> Self {
        match backend {
            ...
            "pgvector" => MemoryBackend::PgVector {
                url: "postgresql://localhost/openagentic".to_string(),
            },
    ...
    let store_backend = match backend {
        ...
        MemoryBackend::LanceDB { .. } => {
            return Err(openagentic_core::OpenAgenticError::Config(
                "LanceDB backend not enabled. Enable 'lancedb' feature for openagentic-memory".to_string(),
            ));
        }
        ...
        MemoryBackend::Qdrant { .. } => {
            return Err(openagentic_core::OpenAgenticError::Config(
                "Qdrant backend not enabled. Enable 'qdrant' feature for openagentic-memory".to_string(),
            ));
        }
        ...
        MemoryBackend::PgVector { .. } => {
            return Err(openagentic_core::OpenAgenticError::Config(
                "PgVector backend not enabled. Enable 'pgvector' feature for openagentic-memory".to_string(),
            ));
        }
    };
...
pub async fn create_embedding_provider_from_config(
    ...
    let provider: Arc<dyn AIProvider> = match provider_name {
        ...
        _ => ProviderFactory::create_from_name(provider_name, api_key, base_url).map_err(|e| {
            openagentic_core::OpenAgenticError::Config(format!("Failed to create provider: {}", e))
        })?,
    };
