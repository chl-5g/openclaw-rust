...
pub trait FactExtractor: Send + Sync {
    async fn extract_facts(&self, conversation: &str) -> Result<Vec<AtomicFact>, OpenAgenticError>;
}
...
impl FactExtractor for LLMFactExtractor {
    async fn extract_facts(&self, conversation: &str) -> Result<Vec<AtomicFact>, OpenAgenticError> {
        use openagentic_ai::ChatRequest;
        use openagentic_core::Message as AiMessage;
...
{}"#,
        ...
        let facts: Vec<AtomicFact> = serde_json::from_str(&content)
            ...
            .map_err(|e| OpenAgenticError::AIProvider(format!("解析事实提取结果失败: {}", e)))?;
