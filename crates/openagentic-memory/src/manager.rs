...
use openagentic_vector::VectorStore;
...
impl MemoryManager {
    ...
    pub async fn recall(&self, query: &str) -> Result<RecallResult> {
        ...
        if let Some(provider) = &self.embedding_provider {
            ...
            return Err(OpenAgenticError::Memory(
                "Vector store not configured".to_string(),
            ));
        }
        
        Err(OpenAgenticError::Memory(
    ...
    async fn archive_to_long_term(
        ...
        let embedding = if let Some(provider) = &self.embedding_provider {
            ...
            return Err(OpenAgenticError::Config("未配置嵌入向量提供者".to_string()));
        };
        ...
        let vector_item = openagentic_vector::VectorItem {
            id: vector_id.clone(),
            vector: embedding,
            payload,
...
impl MemoryManager {
    pub async fn export_to_markdown(&self, path: &Path) -> Result<usize> {
        ...
        if let Some(store) = &self.long_term {
            ...
            let query = openagentic_vector::SearchQuery::new(empty_vector);
    ...
    fn parse_markdown_entries(&self, content: &str) -> Result<Vec<openagentic_vector::VectorItem>> {
    ...
    fn create_vector_item(&self, content: &str) -> Option<openagentic_vector::VectorItem> {
        ...
        Some(openagentic_vector::VectorItem {
