...
use openagentic_ai::{AIProvider, ChatRequest, ChatResponse};
...
impl AICompressProvider for AIProviderCompressAdapter {
    async fn generate_summary(&self, messages: &[Message]) -> Result<String> {
        if messages.is_empty() {
            return Err(OpenAgenticError::AIProvider("没有消息可摘要".to_string()));
        }
        ...
        let user_message = Message::new(Role::User, vec![openagentic_core::Content::Text {
            text: prompt,
        }]);
