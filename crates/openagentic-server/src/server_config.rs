use openagentic_agent::AgentsConfig;
use openagentic_core::config::ChannelSettings;
use openagentic_device::DevicesConfig;
use openagentic_memory::workspace_config::WorkspacesConfig;
...
impl ServerConfig {
    ...
    fn load_core_config(config_dir: &Path) -> std::io::Result<CoreConfig> {
        ...
        // 回退到旧的 openagentic.json（向后兼容）
        let legacy_path = config_dir.join("openagentic.json");
