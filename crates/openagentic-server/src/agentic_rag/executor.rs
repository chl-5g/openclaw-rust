...
use openagentic_core::{Message, Result};
...
pub struct MemoryRetrievalExecutor {
    memory_manager: Arc<openagentic_memory::MemoryManager>,
    source_type: SourceType,
}

impl MemoryRetrievalExecutor {
    pub fn new(memory_manager: Arc<openagentic_memory::MemoryManager>) -> Self {
        Self {
            memory_manager,
...
impl WebRetrievalExecutor {
    async fn search_with_api(
        ...
        let response = client
            ...
            .send()
            .await
            .map_err(|e| openagentic_core::OpenAgenticError::Network(e.to_string()))?;

        let results: Vec<serde_json::Value> = response
            .json()
            .await
            .map_err(|e| openagentic_core::OpenAgenticError::Parse(e.to_string()))?;
...
impl RetrievalExecutor for FileRetrievalExecutor {
    async fn execute(&self, query: &str, config: &ExecutorConfig) -> Result<Vec<RetrievalResult>> {
        let base = self.base_path.as_ref().ok_or_else(|| {
            openagentic_core::OpenAgenticError::Config("FileRetrievalExecutor: base_path not set".to_string())
        })?;
...
pub struct VectorDBRetrievalExecutor {
    vector_store: Arc<dyn openagentic_vector::VectorStore>,
    embedding_provider: Arc<dyn openagentic_memory::embedding::EmbeddingProvider>,
    source_type: SourceType,
...
impl RetrievalExecutor for VectorDBRetrievalExecutor {
    async fn execute(&self, query: &str, config: &ExecutorConfig) -> Result<Vec<RetrievalResult>> {
        ...
        let search_result = self
            .vector_store
            .search(openagentic_vector::SearchQuery {
...
pub struct LlmReranker {
    llm: Arc<dyn openagentic_ai::AIProvider>,
}

impl LlmReranker {
    pub fn new(llm: Arc<dyn openagentic_ai::AIProvider>) -> Self {
        Self { llm }
    }
}
...
impl Reranker for LlmReranker {
    async fn rerank(&self, query: &str, results: &mut Vec<RetrievalResult>) -> Result<()> {
        ...
        let request = openagentic_ai::ChatRequest::new("default", vec![prompt]);
