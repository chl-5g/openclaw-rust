...
use openagentic_agent::squad::{Squad, SquadRegistry, SquadType};
use openagentic_memory::factory::MemoryBackend;

pub struct SquadService {
    squad_registry: Arc<SquadRegistry>,
    agent_memory: Arc<openagentic_agent::squad::AgentMemoryService>,
    squad_memory: Arc<openagentic_agent::squad::SquadMemoryService>,
}
...
impl SquadService {
    pub async fn new(
        ...
        let agent_mem = Arc::new(openagentic_agent::squad::AgentMemoryService::new(
            memory_backend.clone(),
        ));
        let squad_mem = Arc::new(openagentic_agent::squad::SquadMemoryService::new(
            memory_backend,
        ));
    ...
    async fn parse_and_create_squad(
        ...
        if let Some(members_str) = metadata.get("members") {
            ...
            for member in members {
                ...
                if !member.is_empty() {
                    squad = squad.with_member(openagentic_agent::squad::SquadMember::new(
                        member.to_string(),
                        openagentic_agent::squad::SquadRole::Executor,
                    ));
                }
            }
        }
    ...
    pub fn agent_memory_service(&self) -> Arc<openagentic_agent::squad::AgentMemoryService> {
        self.agent_memory.clone()
    }

    pub fn squad_memory_service(&self) -> Arc<openagentic_agent::squad::SquadMemoryService> {
...
mod tests {
    ...
    use openagentic_core::{Message, Result as OpenAgenticResult};
    use openagentic_memory::factory::MemoryBackend;
    use openagentic_memory::recall::RecallResult;
    use openagentic_memory::types::{MemoryItem, MemoryRetrieval};
    ...
    impl MemoryBackend for MockMemoryBackend {
        async fn store(&self, _memory: MemoryItem) -> OpenAgenticResult<()> {
            Ok(())
        }
        async fn recall(&self, _query: &str) -> OpenAgenticResult<RecallResult> {
            Ok(RecallResult {
                items: Vec::new(),
                query: String::new(),
                total_found: 0,
        ...
        async fn add(&self, _message: Message) -> OpenAgenticResult<()> {
            Ok(())
        }
        async fn retrieve(&self, _query: &str, _limit: usize) -> OpenAgenticResult<MemoryRetrieval> {
