...
use openagentic_device::{CameraManager, DeviceCapabilities, ScreenManager, SensorType};
use std::sync::Arc;
...
#[async_trait]
impl openagentic_tools::Tool for CameraTool {
    fn name(&self) -> &str {
        "hardware_camera"
    }
    ...
    async fn execute(&self, args: serde_json::Value) -> OpenAgenticResult<serde_json::Value> {
        let action = args
            .get("action")
            .and_then(|v| v.as_str())
            .unwrap_or("capture");
...
#[async_trait]
impl openagentic_tools::Tool for ScreenTool {
    fn name(&self) -> &str {
        "hardware_screenshot"
    }
