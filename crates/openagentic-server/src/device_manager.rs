use openagentic_device::DevicesConfig;
use std::sync::Arc;

pub struct DeviceManager {
    registry: Arc<openagentic_device::DeviceRegistry>,
    devices_config: Arc<DevicesConfig>,
}

impl DeviceManager {
    pub fn new(config: DevicesConfig) -> Self {
        let registry = openagentic_device::get_or_init_global_registry().clone();
        Self {
            registry,
    ...
    pub async fn init(&self) -> openagentic_core::Result<()> {
        ...
        self.registry.init().await.map_err(|e| {
            openagentic_core::OpenAgenticError::Config(format!("Device registry init failed: {}", e))
        })?;
    ...
    async fn load_custom_devices(&self) -> openagentic_core::Result<()> {
        for device_config in &self.devices_config.custom_devices {
            ...
            let capabilities = if let Some(_caps_config) = &device_config.capabilities {
                openagentic_device::DeviceCapabilities::default()
            } else {
                openagentic_device::DeviceCapabilities::default()
            };

            let device = openagentic_device::DeviceBuilder::new(
            ...
            .config(openagentic_device::DeviceConfig {
            ...
            let handle = device.to_handle(openagentic_device::DeviceStatus::Online);
            self.registry.register(handle).await.map_err(|e| {
                openagentic_core::OpenAgenticError::Config(format!("Register device failed: {}", e))
            })?;
    ...
    async fn load_plugins(&self) -> openagentic_core::Result<()> {
    ...
    fn parse_platform(&self, s: &str) -> openagentic_core::Result<openagentic_device::Platform> {
        use openagentic_device::Platform;
    ...
    fn parse_category(&self, s: &str) -> openagentic_core::Result<openagentic_device::ComputeCategory> {
        use openagentic_device::ComputeCategory;
    ...
    pub fn registry(&self) -> &Arc<openagentic_device::DeviceRegistry> {
        &self.registry
    }

    pub fn get_capabilities(&self) -> openagentic_device::DeviceCapabilities {
