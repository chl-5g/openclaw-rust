...
use openagentic_core::Result as OpenAgenticResult;
use openagentic_core::{Content, Message, OpenAgenticError, Role};
...
use openagentic_memory::recall::{RecallItem as MemoryRecallItem, RecallResult as MemoryRecallResult};
use openagentic_memory::types::{MemoryContent, MemoryItem, MemoryRetrieval};
use openagentic_security::{SecurityPipeline, PipelineResult};
...
impl MemoryPort for MemoryPortAdapter {
    async fn add(&self, entry: MemoryEntry) -> OpenAgenticResult<()> {
        let message = Message {
            id: uuid::Uuid::new_v4(),
            role: Role::User,
    ...
    async fn retrieve(&self, query: &str, limit: usize) -> OpenAgenticResult<Vec<MemoryEntry>> {
    ...
    async fn recall(&self, context: &str, _limit: usize) -> OpenAgenticResult<Vec<RecallItem>> {
    ...
    async fn get_context(&self) -> OpenAgenticResult<Vec<Message>> {
...
impl ToolPort for ToolPortAdapter {
    async fn execute(
        ...
        if !self.registry.has_tool(tool_name) {
            return Err(OpenAgenticError::Tool(format!(
...
impl DevicePort for DevicePortAdapter {
    async fn list_cameras(&self) -> OpenAgenticResult<Vec<CameraInfo>> {
    ...
    async fn capture_camera(&self, camera_id: &str, path: &str) -> OpenAgenticResult<String> {
        let result = self
            .manager
            .capture_camera(camera_id)
            .await
            .map_err(|e| OpenAgenticError::Config(e.to_string()))?;
        
        if result.success {
            Ok(result.data.unwrap_or_else(|| path.to_string()))
        } else {
            Err(OpenAgenticError::Config(result.error.unwrap_or_else(|| "Capture failed".to_string())))
        }
    }

    async fn list_screens(&self) -> OpenAgenticResult<Vec<ScreenInfo>> {
        let capabilities = self.manager.list_capabilities().await;
    ...
    async fn capture_screen(&self, screen_id: &str, path: &str) -> OpenAgenticResult<String> {
        let result = self
            .manager
    ...
    async fn get_location(&self) -> OpenAgenticResult<LocationInfo> {
        Ok(LocationInfo {
    ...
    async fn start_location_tracking(&self) -> OpenAgenticResult<()> {
        Ok(())
    }

    async fn stop_location_tracking(&self) -> OpenAgenticResult<()> {
        Ok(())
    }
}
