...
use openagentic_agent::aieos::{AIEOSParser, AIEOSPromptGenerator};
use openagentic_agent::sessions::{MemorySessionStorage, SessionManager};
use openagentic_agent::task::TaskOutput;
use openagentic_agent::task::{TaskInput, TaskRequest, TaskType};
use openagentic_agent::{Agent, AgentConfig as OpenclawAgentConfig, AgentInfo, AgentType, BaseAgent};
use openagentic_ai::AIProvider;
use openagentic_canvas::CanvasManager;
use openagentic_channels::{ChannelManager, ChannelMessage, SendMessage, register_channels_from_config};
use openagentic_core::{Config, Content, Message, OpenAgenticError, Result, Role};

use openagentic_memory::factory::{MemoryBackend, HybridMemoryBackend};
use openagentic_memory::MemoryConfig;
...
impl MemoryBackend for MemoryManagerAdapter {
    async fn store(&self, memory: openagentic_memory::types::MemoryItem) -> openagentic_core::Result<()> {
        let content = memory.content.to_text();
        let msg = openagentic_core::Message::user(content);
        let mut manager = self.manager.lock().await;
        manager.add(msg).await
    }

    async fn recall(&self, query: &str) -> openagentic_core::Result<openagentic_memory::recall::RecallResult> {
        let manager = self.manager.lock().await;
        manager.recall(query).await
    }

    async fn add(&self, message: openagentic_core::Message) -> openagentic_core::Result<()> {
        let mut manager = self.manager.lock().await;
        manager.add(message).await
    }

    async fn retrieve(&self, query: &str, limit: usize) -> openagentic_core::Result<openagentic_memory::types::MemoryRetrieval> {
        let manager = self.manager.lock().await;
...
pub struct ServiceOrchestrator {
    ...
    tool_executor: Arc<RwLock<Option<Arc<openagentic_tools::ToolRegistry>>>>,
    channel_factory: Arc<openagentic_channels::ChannelFactoryRegistry>,
    agentic_rag_engine: Arc<RwLock<Option<Arc<AgenticRAGEngine>>>>,
    device_manager: Arc<RwLock<Option<Arc<openagentic_device::UnifiedDeviceManager>>>>,
    evolution_engine: Arc<RwLock<Option<Arc<openagentic_agent::EvolutionEngine>>>>,
    shared_skill_registry: Arc<RwLock<Option<Arc<openagentic_agent::evo::registry::SharedSkillRegistry>>>>,
    #[cfg(feature = "per_session_memory")]
...
pub struct ChannelServiceState {
    ...
    pub factory: Arc<openagentic_channels::ChannelFactoryRegistry>,
}
...
impl Default for ChannelServiceState {
    fn default() -> Self {
        Self {
            ...
            factory: Arc::new(openagentic_channels::ChannelFactoryRegistry::new()),
        }
    }
}
...
pub struct OrchestratorConfig {
    ...
    pub channel_configs: Option<openagentic_channels::ChannelConfigs>,
...
impl ServiceOrchestrator {
    pub fn new(config: OrchestratorConfig) -> Self {
        ...
        let channel_factory = Arc::new(openagentic_channels::ChannelFactoryRegistry::new());

        let channel_manager = if config.enable_channels {
            openagentic_channels::ChannelManager::with_factory(channel_factory.clone())
        } else {
            openagentic_channels::ChannelManager::new()
        };
        ...
        let (evolution_engine, shared_skill_registry) = if config.enable_evolution {
            let registry = openagentic_agent::evo::registry::SharedSkillRegistry::new();
            let model = config.evolution_model.clone().unwrap_or_else(|| "gpt-4".to_string());
            let engine = openagentic_agent::EvolutionEngine::new()
                .with_model(model);
            (
                Arc::new(RwLock::new(Some(Arc::new(engine)))) as Arc<RwLock<Option<Arc<openagentic_agent::EvolutionEngine>>>>,
                Arc::new(RwLock::new(Some(Arc::new(registry)))) as Arc<RwLock<Option<Arc<openagentic_agent::evo::registry::SharedSkillRegistry>>>>,
            )
        } else {
            (
    ...
    pub async fn inject_dependencies(
        ...
        tool_registry: Option<Arc<openagentic_tools::ToolRegistry>>,
        device_manager: Option<Arc<openagentic_device::UnifiedDeviceManager>>,
    ) {
        {
        ...
        for agent in agents {
            ...
            let memory_port: Option<Arc<dyn openagentic_agent::ports::MemoryPort>> =
            ...
            let tool_port: Option<Arc<dyn openagentic_agent::ports::ToolPort>> =
                tool_lock.clone().map(|t| {
                    Arc::new(ToolRegistryAdapter::new(t.clone()))
                        as Arc<dyn openagentic_agent::ports::ToolPort>
                });

            let device_port: Option<Arc<dyn openagentic_agent::ports::DevicePort>> =
                device_lock.clone().map(|d| {
                    Arc::new(DevicePortAdapter::new(d))
                        as Arc<dyn openagentic_agent::ports::DevicePort>
                });
    ...
    pub async fn inject_ports(
        &self,
        ai_port: Option<Arc<dyn openagentic_agent::ports::AIPort>>,
        memory_port: Option<Arc<dyn openagentic_agent::ports::MemoryPort>>,
        security_port: Option<Arc<dyn openagentic_agent::ports::SecurityPort>>,
        tool_port: Option<Arc<dyn openagentic_agent::ports::ToolPort>>,
        device_port: Option<Arc<dyn openagentic_agent::ports::DevicePort>>,
    ) {
        let agents: Vec<Arc<dyn Agent>> = {
            let agents = self.agent_service.agents.read().await;
            agents.values().cloned().collect()
    ...
    pub async fn list_sessions(
        &self,
        agent_id: Option<&str>,
        state: Option<openagentic_agent::sessions::SessionState>,
    ) -> openagentic_agent::Result<Vec<openagentic_agent::sessions::Session>> {
        let agent_id: Option<openagentic_agent::types::AgentId> = agent_id.map(|s| s.to_string());
    ...
    pub async fn create_session(
        ...
        name: String,
        agent_id: String,
        scope: openagentic_core::session::SessionScope,
        channel_type: Option<String>,
    ) -> openagentic_agent::Result<openagentic_agent::sessions::Session> {
        let peer_id = match &scope {
            openagentic_core::session::SessionScope::Main => None,
            openagentic_core::session::SessionScope::PerPeer => None,
            openagentic_core::session::SessionScope::PerChannelPeer => None,
            openagentic_core::session::SessionScope::PerAccountChannelPeer => None,
        };
    ...
    pub async fn close_session(&self, session_id: &str) -> openagentic_agent::Result<()> {
        let uuid = uuid::Uuid::parse_str(session_id).map_err(|_| {
            openagentic_agent::OpenAgenticError::Config(format!("Invalid session ID: {}", session_id))
        })?;
    ...
    pub async fn get_session(
        &self,
        session_id: &str,
    ) -> openagentic_agent::Result<Option<openagentic_agent::sessions::Session>> {
        let uuid = uuid::Uuid::parse_str(session_id).map_err(|_| {
            openagentic_agent::OpenAgenticError::Config(format!("Invalid session ID: {}", session_id))
        })?;
    ...
    pub async fn process_message(
        ...
        let agent = self
            .get_agent(agent_id)
            .await
            .ok_or_else(|| OpenAgenticError::Config(format!("Agent not found: {}", agent_id)))?;
        ...
        if result.status == openagentic_agent::task::TaskStatus::Failed {
    ...
    async fn extract_output(&self, result: openagentic_agent::TaskResult) -> Result<String> {
    ...
    async fn try_evolution(&self, agent_id: &str, task: &openagentic_agent::TaskRequest) -> Option<bool> {
        use openagentic_agent::evo::registry::DynamicSkill;
        ...
        if result.status == openagentic_agent::EvolutionStatus::Completed {
    ...
    pub async fn process_channel_message(
        ...
        let agent_id = self
            ...
            .ok_or_else(|| OpenAgenticError::Config("No agent configured".to_string()))?;
        ...
        let channel_msg = ChannelMessage {
            ...
            channel_type: openagentic_channels::ChannelType::WebChat,
    ...
    pub async fn process_chat_stream(
        ...
        use openagentic_ai::types::ChatRequest;
        use tokio::sync::mpsc;
    ...
    pub async fn delete_channel(&self, name: &str) -> openagentic_core::Result<()> {
        let manager = self.channel_service.manager.read().await;
    ...
    pub async fn agent_generate_to_canvas(
        ...
        if !self.config.enable_canvas {
            return Err(OpenAgenticError::Config(
    ...
    pub async fn init_agents_from_config(&self, config: &crate::server_config::ServerConfig) -> Result<()> {
        ...
        for agent_cfg in &agents_config.list {
            let mut openagentic_cfg = OpenclawAgentConfig::new(
                agent_cfg.id.clone(),
                agent_cfg.id.clone(),
                ...
                && aieos_path.exists()
            {
                match AIEOSParser::from_file(aieos_path) {
                    Ok(aieos) => {
                        ...
                        openagentic_cfg = openagentic_cfg.with_system_prompt(system_prompt);
                        tracing::info!(
            ...
            let agent = Arc::new(BaseAgent::new(openagentic_cfg)) as Arc<dyn Agent>;
...
mod tests {
    ...
    async fn test_orchestrator_inject_ports() {
        ...
        use openagentic_agent::Agent;
        use openagentic_agent::mock::mock::MockAiProvider;
        use openagentic_memory::MemoryManager;
        use openagentic_security::SecurityPipeline;
