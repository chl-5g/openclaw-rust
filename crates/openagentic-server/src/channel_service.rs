...
use openagentic_channels::{Channel, ChannelManager, ChannelMessage, SendMessage};
...
impl ChannelService {
    ...
    pub async fn start_all(&self) -> openagentic_core::Result<()> {
        let manager = self.manager.read().await;
        manager.start_all().await
    }

    pub async fn stop_all(&self) -> openagentic_core::Result<()> {
        let manager = self.manager.read().await;
    ...
    pub async fn send_message(
        ...
        channel_name: &str,
        message: SendMessage,
    ) -> openagentic_core::Result<ChannelMessage> {
        let manager = self.manager.read().await;
    ...
    pub async fn broadcast(
        &self,
        message: SendMessage,
    ) -> openagentic_core::Result<Vec<ChannelMessage>> {
        let manager = self.manager.read().await;
