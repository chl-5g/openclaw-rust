...
pub type AcpResult<T> = std::result::Result<T, OpenAgenticError>;
...
impl AcpService {
    ...
    pub async fn handle_message(&self, content: &str, conversation_id: &str, user_id: Option<&str>) -> AcpResult<String> {
        ...
        if route_result.target_agent.is_empty() {
            return Err(OpenAgenticError::Unknown("No agent found to handle the message".to_string()));
        }
        ...
        if response.success {
            ...
            Err(OpenAgenticError::Execution(response.error.unwrap_or_else(|| "Unknown error".to_string())))
        }
    }
    ...
    pub async fn call_agent(&self, agent_id: &str, request: AcpRequest, context_id: Option<&str>) -> AcpResult<AcpResponse> {
        ...
        Err(OpenAgenticError::Unknown(format!("Agent {} not found", agent_id)))
    }
    ...
    async fn call_remote_agent(&self, endpoint: &str, request: AcpRequest, context_id: Option<&str>) -> AcpResult<AcpResponse> {
        ...
        let envelope = AcpEnvelope {
            ...
            from: "openagentic-server".to_string(),
            to: None,
            conversation_id: context_id.map(String::from),
            payload: serde_json::to_value(&request).map_err(|e| OpenAgenticError::Serialization(e))?,
        ...
        let response = client
            ...
            .send()
            .await
            .map_err(|e| OpenAgenticError::Network(e.to_string()))?;

        let envelope: AcpEnvelope = response.json().await
            .map_err(|e| OpenAgenticError::Serialization(serde_json::from_str::<serde_json::Value>(&e.to_string()).unwrap_err()))?;

        let acp_response: AcpResponse = serde_json::from_value(envelope.payload)
            .map_err(|e| OpenAgenticError::Serialization(e))?;

        Ok(acp_response)
    }

    pub async fn get_context(&self, context_id: &str) -> Option<openagentic_acp::SharedContext> {
        self.context_manager.get(context_id).await
    }

    pub async fn create_context(&self, conversation_id: impl Into<String>) -> openagentic_acp::SharedContext {
