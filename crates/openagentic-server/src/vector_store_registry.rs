...
use openagentic_core::config::{LanceDbConfig, MilvusConfig, QdrantConfig, VectorBackend};
use openagentic_vector::{init_default_factories, get_factory, BackendConfig, StoreBackend, VectorStore};
...
pub fn create_vector_store(backend: &StoreBackend) -> Option<Arc<dyn VectorStore>> {
    match backend {
        StoreBackend::Memory => {
            Some(Arc::new(openagentic_vector::MemoryStore::new()) as Arc<dyn VectorStore>)
        }
        _ => None,
    }
}
...
mod tests {
    ...
    async fn test_vector_store_registry_register_and_create() {
        ...
        let store = Arc::new(openagentic_vector::MemoryStore::new()) as Arc<dyn VectorStore>;
