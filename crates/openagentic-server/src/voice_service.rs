...
use openagentic_voice::{SpeechToText, TalkModeConfig, TextToSpeech, VoiceAgent};
...
impl VoiceService {
    ...
    pub async fn speech_to_text(
        ...
        audio_data: &[u8],
        language: Option<&str>,
    ) -> openagentic_core::Result<String> {
        ...
        if let Some(stt_provider) = stt.as_ref() {
            ...
            Err(openagentic_core::OpenAgenticError::Execution(
                "STT not initialized".to_string(),
            ))
        }
    }

    pub async fn start_voice(&self) -> Option<openagentic_core::Result<()>> {
    ...
    pub async fn stop_voice(&self) -> Option<openagentic_core::Result<()>> {
        let agent = self.voice_agent.read().await;
        if let Some(voice_agent) = agent.as_ref() {
    ...
    pub async fn speak(&self, text: &str) -> Option<openagentic_core::Result<Vec<u8>>> {
        let agent = self.voice_agent.read().await;
        if let Some(voice_agent) = agent.as_ref() {
    ...
    pub async fn process_audio(&self, audio_data: &[u8]) -> Option<openagentic_core::Result<String>> {
        let agent = self.voice_agent.read().await;
        if let Some(voice_agent) = agent.as_ref() {
