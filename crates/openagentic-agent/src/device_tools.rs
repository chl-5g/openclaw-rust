...
pub struct CameraTool {
    capabilities: Arc<openagentic_device::DeviceCapabilities>,
}

impl CameraTool {
    pub fn new(capabilities: Arc<openagentic_device::DeviceCapabilities>) -> Self {
        Self { capabilities }
    }
}
        ...
        match params.action.as_str() {
            "capture" => Ok(DeviceToolResult {
                ...
                data: serde_json::json!({
                    "camera_available": self.capabilities.sensors.contains(&openagentic_device::SensorType::Camera),
    ...
    async fn execute(&self, params: DeviceToolParams) -> Result<DeviceToolResult> {
        match params.action.as_str() {
            "get_location" => Ok(DeviceToolResult {
                success: true,
                data: serde_json::json!({
                    "location_available": self.capabilities.sensors.contains(&openagentic_device::SensorType::Gps),
                    "latitude": 0.0,
                    "longitude": 0.0
                }),
...
impl DeviceToolRegistry {
    ...
    pub async fn execute(
        ...
        Err(openagentic_core::OpenAgenticError::Config(format!(
            "Tool not found: {}",
            tool_name
        )))
    }
