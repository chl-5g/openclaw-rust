...
use openagentic_ai::{AIProvider, ChatRequest};
use openagentic_core::{Content, Message, Result, session::SessionScope};
use openagentic_memory::MemoryManager;
use openagentic_security::{PipelineResult, SecurityPipeline};
use openagentic_tools::ToolResult as OpenAgenticToolResult;
...
pub trait Agent: Send + Sync {
    ...
    async fn execute_tool(
        &self,
        executor: &openagentic_tools::SkillRegistry,
        name: &str,
        arguments: &serde_json::Value,
    ) -> std::result::Result<OpenAgenticToolResult, String>;
...
pub struct BaseAgent {
    ...
    tool_executor: Arc<tokio::sync::RwLock<Option<Arc<openagentic_tools::SkillRegistry>>>>,
    tool_registry: Arc<tokio::sync::RwLock<Option<Arc<openagentic_tools::ToolRegistry>>>>,
...
impl BaseAgent {
    ...
    pub async fn save_session_messages(&self, messages: &[openagentic_core::Message]) -> crate::Result<()> {
        let storage_guard = self.session_storage.read().await;
...
impl Agent for BaseAgent {
            ...
            Ok(response) => {
                ...
                Ok(TaskResult {
                    ...
                    output: Some(TaskOutput::Message {
                        message: openagentic_core::Message::assistant(final_output.clone()),
                    }),
                    error: None,
                    started_at,
    ...
    async fn execute_tool(
        ...
        if skill_found {
            ...
            return Ok(OpenAgenticToolResult::success(
...
mod tests {
    ...
    async fn test_agent_inject_dependencies() {
        ...
        let memory_manager = Arc::new(openagentic_memory::MemoryManager::default());
        agent
            .inject_dependencies(
                mock_provider.clone(),
                Some(memory_manager),
                Arc::new(openagentic_security::SecurityPipeline::default()),
                None,
            )
            .await;
