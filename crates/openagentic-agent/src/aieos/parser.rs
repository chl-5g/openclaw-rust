...
impl AIEOSParser {
    pub fn from_json(json: &str) -> Result<AIEOS> {
        let aieos: AIEOS = serde_json::from_str(json)
            .map_err(|e| OpenAgenticError::Config(format!("AIEOS parse error: {}", e)))?;
    ...
    pub fn from_file(path: &std::path::Path) -> Result<AIEOS> {
        let content = std::fs::read_to_string(path)
            .map_err(|e| OpenAgenticError::Config(format!("Failed to read AIEOS file: {}", e)))?;
    ...
    pub fn validate(aieos: &AIEOS) -> Result<()> {
        ...
        if aieos.motivations.core_drive.is_empty() {
            return Err(OpenAgenticError::Config("Core drive is required".to_string()));
        }
