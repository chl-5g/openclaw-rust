...
impl Orchestrator {
    ...
    pub fn with_device_manager(
        mut self,
        manager: Arc<openagentic_device::UnifiedDeviceManager>,
    ) -> Self {
    ...
    /// 获取设备管理器
    pub fn get_device_manager(&self) -> Option<Arc<openagentic_device::UnifiedDeviceManager>> {
    ...
    pub async fn handle_message(&self, message: Message) -> Result<Message> {
        ...
        match result.output {
            ...
            None => Err(OpenAgenticError::Unknown("No output from agent".to_string())),
        }
    }
    ...
    async fn execute_with_skill(
        ...
        let registry = self.shared_skill_registry.as_ref()
            .ok_or_else(|| openagentic_core::OpenAgenticError::Config("Skill registry not initialized".into()))?;

        let skill = registry.get_skill(&skill_id).await
            .ok_or_else(|| openagentic_core::OpenAgenticError::Config(format!("Skill '{}' not found", skill_id)))?;
        ...
        let result = match skill.skill_type {
            crate::evo::registry::SkillType::Code => {
                if let Some(code) = &skill.code {
                    ...
                    Err(openagentic_core::OpenAgenticError::Config("Code skill has no code".into()))
                }
            }
    ...
    pub async fn run_with_graph(
        ...
        let result = executor
            ...
            .await
            .map_err(OpenAgenticError::Execution)?;
