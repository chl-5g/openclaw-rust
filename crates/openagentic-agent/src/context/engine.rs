...
use openagentic_memory::traits::VectorStoreTrait;
...
pub fn extract_content(message: &openagentic_core::Message) -> String {
    message.content.iter().map(|c| {
        match c {
            openagentic_core::Content::Text { text } => text.clone(),
            openagentic_core::Content::Image { url } => format!("[Image: {}]", url),
            openagentic_core::Content::Audio { url } => format!("[Audio: {}]", url),
            openagentic_core::Content::ToolCall { name, .. } => format!("[Tool: {}]", name),
            openagentic_core::Content::ToolResult { content, .. } => content.clone(),
        }
    }).collect::<Vec<_>>().join("\n")
}
