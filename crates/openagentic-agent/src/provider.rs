...
use openagentic_core::{Message, OpenAgenticError, Result};
...
pub struct MemoryManagerWrapper {
    inner: Arc<tokio::sync::Mutex<openagentic_memory::MemoryManager>>,
}

impl MemoryManagerWrapper {
    pub fn new(manager: openagentic_memory::MemoryManager) -> Self {
        Self {
...
pub struct SecurityPipelineWrapper {
    inner: Arc<openagentic_security::SecurityPipeline>,
}

impl SecurityPipelineWrapper {
    pub fn new(pipeline: openagentic_security::SecurityPipeline) -> Self {
        Self {
...
impl SecurityProvider for SecurityPipelineWrapper {
    async fn check_input(&self, session_id: &str, input: &str) -> Result<()> {
        ...
        match result {
            openagentic_security::PipelineResult::Allow => Ok(()),
            openagentic_security::PipelineResult::Block(reason) => Err(OpenAgenticError::Execution(
                format!("Input blocked: {}", reason),
            )),
            openagentic_security::PipelineResult::Warn(reason) => {
    ...
    async fn validate_output(&self, session_id: &str, output: &str) -> Result<()> {
        ...
        if validation.requires_action {
            Err(OpenAgenticError::Execution(format!(
...
#[async_trait]
impl ToolProvider for openagentic_tools::ToolRegistry {
    async fn execute(&self, tool_name: &str, args: Value) -> Result<Value> {
    ...
    fn test_provider_registry_builder_pattern() {
        let registry = ProviderRegistry::new()
            .with_memory(Arc::new(MemoryManagerWrapper::new(
                openagentic_memory::MemoryManager::new(openagentic_memory::MemoryConfig::default()),
            )))
            .with_security(Arc::new(SecurityPipelineWrapper::new(
                openagentic_security::SecurityPipeline::new(
                    openagentic_security::PipelineConfig::default(),
                ),
            )))
            .with_tools(Arc::new(openagentic_tools::ToolRegistry::new()));
