...
use openagentic_voice::{SpeechToText, TalkMode, TalkModeConfig, TextToSpeech, VoiceAgent};
...
impl AgentVoice {
    ...
    pub async fn process_voice_input(&self, audio_data: &[u8]) -> Result<String> {
        let agent = self.voice_agent.as_ref().ok_or_else(|| {
            openagentic_core::OpenAgenticError::Config("Voice agent not initialized".to_string())
        })?;
