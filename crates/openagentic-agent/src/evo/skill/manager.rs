...
impl SkillManager {
    ...
    pub async fn initialize(&self) -> crate::Result<()> {
        tokio::fs::create_dir_all(&self.cache_dir).await
            .map_err(|e| crate::OpenAgenticError::Config(format!("Failed to create cache dir: {}", e)))?;
    ...
    pub async fn install_from_file(&self, path: &str) -> crate::Result<Skill> {
        let content = tokio::fs::read_to_string(path).await
            .map_err(|e| crate::OpenAgenticError::Config(format!("Failed to read file: {}", e)))?;
        ...
        let mut dynamic_skill = loader.parse_skill_md(&content)
            .map_err(|e| crate::OpenAgenticError::Config(format!("Failed to parse skill: {}", e)))?;
        ...
        tokio::fs::write(&skill_path, content).await
            .map_err(|e| crate::OpenAgenticError::Config(format!("Failed to save to cache: {}", e)))?;
    ...
    pub async fn install_from_hub(&self, name: &str) -> crate::Result<Skill> {
        let client = self.hub_client.as_ref()
            .ok_or_else(|| crate::OpenAgenticError::Config("ClawHub client not configured".into()))?;
    ...
    async fn install_from_url(&self, url: &str) -> crate::Result<Skill> {
        let response = reqwest::get(url).await
            .map_err(|e| crate::OpenAgenticError::Config(format!("Failed to fetch URL: {}", e)))?;
        
        let content = response.text().await
            .map_err(|e| crate::OpenAgenticError::Config(format!("Failed to read response: {}", e)))?;
    ...
    pub async fn uninstall(&self, name: &str) -> crate::Result<()> {
        ...
        if skill_path.exists() {
            tokio::fs::remove_file(&skill_path).await
                .map_err(|e| crate::OpenAgenticError::Config(format!("Failed to remove cached file: {}", e)))?;
        }
...
impl ClawHubClient {
    ...
    pub async fn search(&self, query: &str, limit: usize) -> crate::Result<Vec<SearchResult>> {
        ...
        let response = request.send().await
            .map_err(|e| crate::OpenAgenticError::Config(format!("Request failed: {}", e)))?;
        
        if !response.status().is_success() {
            return Err(crate::OpenAgenticError::Config(
                format!("ClawHub API error: {}", response.status())
            ));
        }
        
        let results: Vec<SearchResult> = response.json().await
            .map_err(|e| crate::OpenAgenticError::Config(format!("Failed to parse response: {}", e)))?;
...
mod tests {
    ...
    async fn test_skill_manager_default_cache_dir() {
        ...
        assert!(cache_dir.to_string_lossy().contains("openagentic"));
    }
