...
impl LlmSkillGenerator {
    ...
    pub async fn generate_skill_code(&self, pattern: &TaskPattern) -> Result<String> {
        ...
        let mut request = openagentic_ai::ChatRequest::new(&model, vec![openagentic_core::Message::user(&prompt)]);
...
mod tests {
    use super::*;
    use chrono::Utc;
    use openagentic_ai::providers::BaseProvider;
    use openagentic_ai::ProviderConfig;
