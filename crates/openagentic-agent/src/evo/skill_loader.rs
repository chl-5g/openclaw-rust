...
impl SkillLoader {
    ...
    pub async fn load_from_file(&self, path: &Path) -> Result<DynamicSkill, String> {
        ...
        match format {
            ...
            SkillFormat::OpenAgentic => self.parse_goclaw(&content),
        }
    }
    ...
    pub fn get_skill_paths() -> Vec<String> {
        ...
        paths.push("./openagentic/skills".to_string());
        
        if let Ok(home) = std::env::var("HOME") {
            paths.push(format!("{}/.openagentic/skills", home));
            paths.push(format!("{}/.agentskills/skills", home));
        }
        
        #[cfg(unix)]
        {
            paths.push("/usr/local/share/openagentic/skills".to_string());
        }
    ...
    pub fn parse_skill_md(&self, content: &str) -> Result<DynamicSkill, String> {
        ...
        match format {
            ...
            SkillFormat::GoClaw | SkillFormat::OpenAgentic => {
